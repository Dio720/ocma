...
var q = (r, t, e) => t in r ? w(r, t, {
    ...
    P = (r, t) => {
        ...
        if (K)
            for (var e of K(t)) Gt.call(t, e) && q(r, e, t[e]);
        return r
    },
    Z = (r, t) => wt(r, It(t)),
    Mt = r => w(r, "__esModule", {
...
var qt = (r, t, e) => {
        if (t && typeof t == "object" || typeof t == "function")
            for (let s of Tt(t)) !Q.call(r, s) && s !== "default" && w(r, s, {
                get: () => t[s],
                enumerable: !(e = Rt(t, s)) || e.enumerable
            });
        return r
    },
    g = r => qt(Mt(w(r != null ? St(Ot(r)) : {}, "default", r && r.__esModule && "default" in r ? {
        get: () => r.default,
        enumerable: !0
    } : {
        value: r,
...
var nt = Lt(_ => {
    ...
    var C, D = function() {
        ...
        Ut = function(r) {
            ...
            return U(t, r), t.prototype.next = function() {
...
function et(r = "", t = []) {
    ...
    return tt.concat(t).forEach(o => {
        let a = e.indexOf(o.flag);
        a > 1 && a < e.length - o.num && (o.process(s, e, a), e.splice(a, o.num + 1))
    }), e.includes("-h") && (Bt(r, t), process.exit(0)), s.bots = e.slice(2).filter(o => !o.startsWith("-")), s
}
...
var n = Object.keys(R).reduce((r, t) => Z(P({}, r), {
    [t]: e => Ft(`${R[t]}${e}`)
}), {});
var d = console.log,
    ...
    $ = class {
        constructor(t, e, s) {
            ...
            i(this, "_active");
            I.dirname(s) === "." ? this._proc = (0, N.spawn)(I.join(process.cwd(), s)) : this._proc = (0, N.spawn)(s), this.game.config.messages && (this._msgStream = (0, ot.createWriteStream)(s + ".in")), this.game.config.quiet && (d = () => {}), this._proc.on("error", o => {
	...
	`);
            ...
            }), this._read = st(this._proc.stdout), this._active = !0
        }
        async write(t) {
            if (this._active) {
...
`;
        ...
        async read() {
            if (this._active) {
                ...
                return a === "TIMEOUT" ? (d(`Jogador ${this.id} n\xE3o enviou dados no tempo de ${this.game.config.timeout}ms e foi eliminado.`), Promise.resolve("PLAYER_ERROR")) : a === "ERROR" ? (d(`Jogador ${this.id} teve problemas t\xE9cnicos e foi eliminado.`), Promise.resolve("PLAYER_ERROR")) : (clearTimeout(t), this.game.config.debug && console.debug(`${it} Recebido de ${this.id}: ${a}`), Promise.resolve(a))
            }
        }
        halt() {
...
var T = console.log,
    ...
    O = class {
        constructor(t) {
            ...
            this.config = P(P({}, t), e), this.config.quiet && (T = () => {}), this.config.readfile && this.loadFile(this.config.readfile), this.config.seed || (y(), this.config.seed = v()), T(`Seed da partida ${v()}`), (!this.currPlayerIndex || this.currPlayerIndex < 0 || this.currPlayerIndex > this.config.bots.length - 1) && (this.currPlayerIndex = h(this.config.bots.length)), this.players = this.config.bots.map(s => new $(this, ut.basename(s), s)), this.players.forEach((s, o) => s.color = Vt[o])
        }
        async run() {
            try {
...
`),
        ...
        async forEachPlayer(t) {
            await Promise.all(this.players.map(async e => {
                await t(e)
            }))
        }
        get numPlayers() {
        ...
        async broadcastNonCurrent(t) {
            await this.broadcast(t, this.players.filter(e => e != this.currentPlayer))
        }
...
var L = class {
    ...
    E = class {
        constructor() {
            ...
            let t = () => y() < .5 ? G + h(10, 1) : y() < .7 ? H + h(10, 1) : M + h(10, 1);
            this._cell = [];
            for (let e = 0; e < this.height; e++) {
                this._cell.push([]);
                for (let s = 0; s < this.width; s++) this._cell[e].push(y() <= .3 ? t() : 0)
            }
            this.harbors = [];
        ...
        getFishInfo(t) {
            if (this.isFishingPoint(t)) {
                ...
                switch (e - s) {
                    case G:
                        return {
                            fish: "MULLET", quantity: s
                        };
                    case H:
                        return {
                            fish: "SNAPPER", quantity: s
                        };
                    case M:
                        return {
                            fish: "SEABASS", quantity: s
                        }
                }
            }
            return null
...
var W = class {
    constructor(t = new L(0, 0)) {
        ...
        i(this, "_moves");
        i(this, "_charge");
        this._type = "BOAT", this._pos = t, this._moves = 0, this._charge = [{
            fish: "MULLET",
            quantity: 0
        }, {
            fish: "SNAPPER",
...
var b = class {
        ...
        move(t) {
            let e = p.instance,
                s = P({}, this.pos);
            return t === "LEFT" && s.col--, t === "RIGHT" && s.col++, t === "UP" && s.lin--, t === "DOWN" && s.lin++, e.isInside(s) ? b.isBusy(s) ? new c("BUSY") : (this._boat.moveTo(s), new c("DONE")) : (this._money -= dt, new c("OUT"))
        }
        fish() {
            let t = p.instance,
...
function vt() {
    console.log("Bots");
    let t = f.bots.reduce((e, s, o) => `${e}${k(o)}: ${s.id}
`, "");
    console.log(t), console.log("Pontos de pesca (o digito \xE9 a quantidade)"), console.log(A({
        fish: "MULLET",
        quantity: 0
    }) + ": Tainha"), console.log(A({
        fish: "SNAPPER",
        quantity: 0
    }) + ": Cioba"), console.log(A({
        fish: "SEABASS",
        quantity: 0
    }) + ": Robalo"), console.log(), console.log("P: Porto")
}
    ...
    for (let s = 0; s < r.height; s++) {
        ...
        if (s < t.length) {
            let o = t[s],
                a = A({
                    fish: "MULLET",
                    quantity: o.quantity("MULLET")
                }),
                u = A({
                    fish: "SNAPPER",
                    quantity: o.quantity("SNAPPER")
                }),
                S = A({
                    fish: "SEABASS",
                    quantity: o.quantity("SEABASS")
                });
...
var x = 500,
    ...
    m = new O({
        description: "Neste jogo, voc\xEA controla um barco-pesqueiro, que compete com outros barcos pelos recursos naturais de uma \xE1rea de pesca.",
        loadState: Wt,
        init: kt,
        start: Yt,
        turn: zt,
...
async function zt() {
    ...
    try {
        if (++xt % m.numPlayers == 0 && (console.log(`Faltam ${x} rodadas.`), x--), x >= 0) {
            ...
            for (let a of f.bots) {
                drawBoat(a.id, a.money, a.pos.lin, a.pos.col)
                await m.send(t, new c(a.id, a.pos.lin, a.pos.col))
            }
            writeWatch()
...
function Jt() {
    drawColocacao()
    console.log("COLOCA\xC7\xC3O"), f.bots.sort((r, t) => t.money - r.money).forEach((r, t) => {
...
const COLORS = {
    ...
    robalo: "HotPink",
    cioba: "SeaGreen",
    tainha: "DarkSlateGrey"
}

registerFont( 'open-sans.ttf', { family: "open-sans" } )

const canvas = createCanvas(canvasWidth, canvasHeight)
const canvasCtx = canvas.getContext('2d')

canvasCtx.font = '20px open-sans';
...
function writeWatch() {
    ...
    fs.writeFileSync('./watch.png', buffer)
}

function drawMap(strData) {
    const r = p.instance
    const map = Array(r.height).fill(Array(r.width).fill("0"))
    const array = strData.split("\n")
    ...
    for (let i = 0; i < r.height; i++) {
        for (let j = 0; j < r.width; j++) {
            let label = ""
            if (map[i][j] == "0") {
                canvasCtx.fillStyle = COLORS.water
            } else if (map[i][j] == "1") {
                canvasCtx.fillStyle = COLORS.harbor;
                label = "P"
            } else {
                label = "" + (map[i][j]).charAt(1)
                if (map[i][j].startsWith("3")) canvasCtx.fillStyle = COLORS.robalo
                else if (map[i][j].startsWith("2")) canvasCtx.fillStyle = COLORS.cioba
                else if (map[i][j].startsWith("1")) canvasCtx.fillStyle = COLORS.tainha
            }

            const rectWidth = canvasWidth/r.width
            const rectHeight = canvasHeight/r.height
            
            canvasCtx.fillRect(j * rectWidth, i * rectHeight, rectWidth, rectHeight)
            canvasCtx.fillStyle = "white"
            canvasCtx.fillText(label,j * rectWidth +(rectWidth/2), i * rectHeight +(rectHeight/2))
        } 
    }
    
}

function drawBoat(id, money, x, y) {
    const r = p.instance
    const label = `${id}\nR$${money}`
    ...
    canvasCtx.fillStyle = COLORS.boat
    canvasCtx.fillRect(y * rectWidth, x * rectHeight, rectWidth, rectHeight)
    canvasCtx.fillStyle = "white"
    canvasCtx.fillText(label, y * rectWidth + (rectWidth/2), x * rectHeight + (rectHeight/2))
}

function drawColocacao() {
    canvasCtx.fillStyle = "black"
    canvasCtx.fillRect(0, 0, canvasWidth, canvasHeight)
    
    let text = "COLOCAÇÃO\n\n"
    f.bots.sort((r, t) => t.money - r.money).forEach((r, t) => {
        text += `${t + 1}\xBA: ${r.id} - R$ ${r.money},00\n`
    })
